#ifndef __SYALR_HTTP_HTTP_H__
#define __SYALR_HTTP_HTTP_H__
...
// 通过传入的字符串描述去HttpMethod中匹配，返回匹配到的状态名称
...
public:
    ...
    template<class T>
    T checkGetHeaderAs(const std::string& key, T& val, const T& def = T()) {
    ...
    template<class T>
    T checkGetParamAs(const std::string& key, T& val, const T& def = T()) {
    ...
    template<class T>
    T checkGetCookiesAs(const std::string& key, T& val, const T& def = T()) {
